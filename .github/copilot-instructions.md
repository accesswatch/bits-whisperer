...
- **First-run wizard**: 6-page setup wizard for hardware scan, model recommendations, downloads, provider setup, and preferences
...
src/bits_whisperer/
  ...
  storage/                 # Persistence
    ...
    key_store.py         # OS credential store via keyring (15 provider keys)
  ui/                      # WXPython UI
...
3. **Provider count consistency** — when adding/removing providers, update the count in ALL docs
   (currently 17 providers, 15 API key entries in KeyStore).
